...
# HUMAnN runs too slow, so Cas profiling with HUMAnN is abandoned.
...
do
	...
	do
		echo ${sample}
		gunzip ${INPUT_DIR}/${sample}"_paired_dehost.1.fastq.gz" -k
                gunzip ${INPUT_DIR}/${sample}"_paired_dehost.2.fastq.gz" -k
                gunzip ${INPUT_DIR}/${sample}"_unpaired_dehost.fastq.gz" -k
		...
		cat ${INPUT_1} ${INPUT_2} ${INPUT_UNPAIRED} > ${INPUT_MERGED} 
		rm ${INPUT_1}; rm ${INPUT_2}; rm ${INPUT_UNPAIRED}

		cd ${OUTPUT_DIR}
		humann -r --input ${INPUT_MERGED} --output-basename ${sample} --output ${OUTPUT_DIR}/${sample} --threads 16 --memory-use maximum --metaphlan-options "--bowtie2db /home/zhanggaopu/software/metaphlan/bowtie2db_for_humann --index mpa_vJan21_CHOCOPhlAnSGB_202103 --nproc 16" --bowtie-options "--threads 16" --diamond-options "--threads 8" --nucleotide-database ~/software/humann/humann_database/chocophlan --protein-database ~/software/humann/humann_database/uniref90/uniref
		rm ${INPUT_MERGED}
	done
done
